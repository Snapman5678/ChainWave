...
func AddCustomer(db *sql.DB, userId uuid.UUID, customer models.Customer) error {
	tx, err := db.Begin()
	if err != nil {
		return err
	}

	_, err = tx.Exec(`INSERT INTO customers (id, customer_name, contact_info, location_id, user_id) VALUES (uuid_generate_v4(), $1, $2, $3, $4)`,
		customer.CustomerName, customer.ContactInfo, customer.LocationId, userId)
	if err != nil {
		tx.Rollback()
		return err
	}
...
func AddBusinessAdmin(db *sql.DB, userId uuid.UUID, businessAdmin models.BusinessAdmin) error {
	tx, err := db.Begin()
	if err != nil {
		return err
	}

	_, err = tx.Exec(`INSERT INTO business_admins (id, company_name, contact_info, location_id, user_id) VALUES (uuid_generate_v4(), $1, $2, $3, $4)`,
		businessAdmin.CompanyName, businessAdmin.ContactInfo, businessAdmin.LocationId, userId)
	if err != nil {
		tx.Rollback()
		return err
	}
...
func AddTransporter(db *sql.DB, userId uuid.UUID, transporter models.Transporter) error {
	tx, err := db.Begin()
	if err != nil {
		return err
	}

	_, err = tx.Exec(`INSERT INTO transporters (id, driver_name, vehicle_id, contact_info, location_id, user_id) VALUES (uuid_generate_v4(), $1, $2, $3, $4, $5)`,
		transporter.DriverName, transporter.VehicleId, transporter.ContactInfo, transporter.LocationId, userId)
	if err != nil {
		tx.Rollback()
		return err
	}
...
func AddSupplier(db *sql.DB, userId uuid.UUID, supplier models.Supplier) error {
	tx, err := db.Begin()
	if err != nil {
		return err
	}

	_, err = tx.Exec(`INSERT INTO suppliers (id, supplier_name, contact_info, address, location_id, user_id) VALUES (uuid_generate_v4(), $1, $2, $3, $4, $5)`,
		supplier.SupplierName, supplier.ContactInfo, supplier.Address, supplier.LocationId, userId)
	if err != nil {
		tx.Rollback()
		return err
	}
